...
extern id SubViewForIdentity(NSString* iden, id view)
{
	...
	{
		if ([ [ (MDControlView*)[ [ view subViews ] objectAtIndex:z ] identity ] isEqualToString:iden ])
		{
			ret = [ [ view subViews ] objectAtIndex:z ];
			break;
		}
	}
	return ret;
...
- (id) init
{
	if ((self = [ super init ]))
	{
		...
		background = [ [ NSColor colorWithDeviceRed:0 green:0 blue:0 alpha:1 ]
						retain ];
		strokeColor = [ [ NSColor colorWithCalibratedRed:0 green:0 blue:0 alpha:1 ] retain ];
...
- (id) initWithFrame: (MDRect)rect background: (NSColor*)bkg;
{
	...
	{
		frame = rect;
		background = [ [ NSColor colorWithCalibratedRed:[ bkg redComponent ] green:
...
- (void) setFrame: (MDRect)rect
{
	...
	for (unsigned long z = 0; z < [ subViews count ]; z++)
	{
		MDControlView* view = [ subViews objectAtIndex:z ];
...
- (void) setBackground: (NSColor*)bkg
{
	if (background)
		[ background release ];
	background = [ [ NSColor colorWithDeviceRed:[ bkg redComponent ] green:
				[ bkg greenComponent ] blue:[ bkg blueComponent ] alpha:
						   [ bkg alphaComponent ] ] retain ];
	updateVAO = TRUE;
}
...
- (void) setStrokeColor:(NSColor*)color
{
	if (strokeColor)
		[ strokeColor release ];
	strokeColor = [ [ NSColor colorWithDeviceRed:[ color redComponent ] green:
					[ color greenComponent ] blue:[ color blueComponent ] alpha:
					[ color alphaComponent ] ] retain ];
	updateVAO = TRUE;
}
...
{
	NSColor* backup = [ background retain ];
	if (background)
		[ background release ];
	background = [ [ NSColor colorWithCalibratedRed:red green:[ backup greenComponent ] blue:[ backup blueComponent ] alpha:[ backup alphaComponent ] ] retain ];
	[ backup release ];
}
...
- (void) setGreen: (float)green
{
	...
	if (background)
		[ background release ];
	background = [ [ NSColor colorWithDeviceRed:[ backup redComponent ] green:green
...
- (void) setBlue: (float)blue
{
	...
	if (background)
		[ background release ];
	background = [ [ NSColor colorWithDeviceRed:[ backup redComponent ] green:
...
{
	for (unsigned long z = 0; z < [ subViews count ]; z++)
		[ [ subViews objectAtIndex:z ] beforeDraw ];
	return FALSE;
}
...
{
	for (unsigned long z = 0; z < [ subViews count ]; z++)
		[ [ subViews objectAtIndex:z ] alphaDraw ];
}
