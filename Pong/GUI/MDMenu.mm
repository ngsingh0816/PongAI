...
- (id) init
{
	if ((self = [ super init ]))
	{
		...
		tracking = TRUE;
		alwaysOnTop = TRUE;
		selectionColor = [ MD_MENU_DEFAULT_SELECTION_COLOR retain ];
	}
	return self;
}
...
- (void) drawView
{
	...
	if (!vao[0] || updateVAO)
	{
		...
		if ([ mitems count ] != 0)
		{
			if ([ [ [ mitems objectAtIndex:0 ] subItems ] count ] != 0)
			{
				float height = [ (MDMenuItem*)[ [ (MDMenuItem*)[ mitems objectAtIndex:0 ] subItems ] objectAtIndex:0 ] frame ].height;
				frame.height = [ [ [ mitems objectAtIndex:0 ] subItems ] count ] * height;
				float width = 0;
				for (unsigned long z = 0; z < [ mitems count ]; z++)
				{
					float temp =  [ (MDMenuItem*)[ [ (MDMenuItem*)[ mitems objectAtIndex:0 ] subItems ] objectAtIndex:0 ] frame ].width;
					if (temp > width)
						width = temp;
				}
				frame.width = width;
...
- (void) drawSubItems: (MDMenuItem*)item
{
	...
	for (int z = 0; z < [ sub count ]; z++)
	{
		...
		{
			color = [ NSColor colorWithCalibratedRed:[ color redComponent ] green:[ color greenComponent ] blue:[ color blueComponent ] alpha:currentAlpha ];
			DrawStringColor([ [ sub objectAtIndex:z ] glStr ], NSMakePoint(frm.x + 12,
								frm.y + (frm.height / 2)), NSLeftTextAlignment, 0, color);
		}
		else
		{
...
- (void) drawView
{
	...
	if (!vao[0] || updateVAO)
	{
		...
		if ([ mitems count ] != 0)
		{
			if ([ [ [ mitems objectAtIndex:0 ] subItems ] count ] != 0)
			{
				height = [ (MDMenuItem*)[ [ (MDMenuItem*)[ mitems objectAtIndex:0 ] subItems ] objectAtIndex:0 ] frame ].height;
				frame.height = [ [ [ mitems objectAtIndex:0 ] subItems ] count ] * height;
				float width = 0;
				for (unsigned long z = 0; z < [ mitems count ]; z++)
				{
...
{
	MDPopupMenu* menu = [ [ [ MDPopupMenu alloc ] initWithFrame:MakeRect(location.x - 10, location.y + 5, width, 0) background:MD_MENU_DEFAULT_COLOR ] autorelease ];
